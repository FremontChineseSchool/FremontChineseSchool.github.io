...
`費利蒙` is revealed **stroke-by-stroke** in 王漢宗中行書 (running-script) brush
...
- `scripts/build_glyph_data.py` extracts the outlines from the GPL font
  **王漢宗中行書** (`wt071`, re-downloaded on demand) → writes the lean
...
to more characters, align them and add to `ANIMATED`. Licensing caveat: 王漢宗中行書
is GPL (with an old 文鼎/Arphic provenance question) — only the 7 glyph outlines are
embedded, not the font; confirm this is acceptable before a public launch.
